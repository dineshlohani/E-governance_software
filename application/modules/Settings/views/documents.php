...
if(isset($result))
{
    $action_page = base_url()."add-document/".$result->id;
}
else
{
    $action_page = base_url()."add-document";
}
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="col-12 col-md-6 offset-md-3" >
                    <div class="form-group">
                        <label>पत्रको किसिम</label>
                        <select name="patra_id" class="select2 form-control" required>
                            <option>छान्नुहोस्</option>
                            <?php foreach($patras as $patra):?>
                                <option value="<?= $patra->id ?>"
                                    <?php
                                        if(isset($result) && $result->patra_id == $patra->id)
                                        {
            ...
            <div class="box box-primary"  style="margin-left: 10px">
                <div class="box-header with-border">
                    <center><h4 class="box-title">कागजातहरु</h4></center>
                </div>
                <table id="table1" class="table table-bordered table-striped">
                    ...
                    <?php
                        ...
                        foreach ($documents as $document){
                            $patra = Modules::run('Settings/getPatraItem',$document->patra_id)
                    ?>
                        <tr>
                            <td><?= $i?></td>
                            <td><?= $document->name?></td>
                            <td><?= $patra->name ?></td>
                            <td>
                                <a href="<?=base_url()?>add-document/<?=$document->id?>">
                                <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                </a>&nbsp;&nbsp;
                                <a href="<?=base_url()?>department/<?=$document->id?>">
