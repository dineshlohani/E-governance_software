...
class Unmarried extends MX_Controller {
...

    {
        ...
        $this->load->model('English/Mdl_unmarried');
    ...
    {  
        ...
        $data['title']  = "Unmarried Verification | Dashboard";

        $data['result']  = $this->Mdl_unmarried->getAll();

        $this->load->view('User/header',$data);

        $this->load->view('unmarried/list');
    ...
    { 
        ...
        $data1['title']         = "Unmarried Verification | Create";

        $this->load->view('User/header',$data1);

        $this->load->view('unmarried/create',$data);
    ...
    public function saveDetails() {
        ...
        if(isset($_POST['submit'])) {
            ...
            $save = array(
                ...
                'former_vdc'                => $former_vdc,

                'former_ward'               => $former_ward,
            ...
            if($id = $this->Mdl_unmarried->save($save)) {

                $this->session->set_flashdata('msg'," Success");

                redirect('unmarried-en/detail/'.$id);

            } else {

                $this->session->set_flashdata('err_msg'," समस्या आयो |");

                redirect('unmarried-en/create');
    ...
    public function darta_view() {
        ...
        if(empty($id)){
            ...
            redirect('unmarried-en');

        }

        Modules::run("User/checkWardLogin");

        $data['title']                  = "Unmarried Verification | Details";

        $data['page']                   = 'unmarried/details';
        ...
        $data['result']                 = $this->Mdl_unmarried->getById($id);

        $data['sources']                = $this->Mdl_unmarried->getRelationShipDetails($id);
    ...
    public function edit_details() {
        ...
        $data['detail']         = $this->Mdl_unmarried->getById($id);
    ...
    public function update() {
         ...
         if(isset($_POST['submit'])) {
            ...
            if($this->Mdl_unmarried->update($id,$save)) {
    ...
    public function darta_details() {
        ...
        $result     = $this->Mdl_unmarried->getById($id);
        ...
        if($this->Mdl_unmarried->update($id,$data))
...

    {
...

        {
            ...
            redirect("unmarried-en/detail/".$result->id);
    ...
    public function print() {
        ...
        $data['result'] = $result   = $this->Mdl_unmarried->getById($id);
       ...
       //  }
        ...
        $this->load->view('unmarried/print',$data);
    ...
    public function removeData($main_id,$id) {
        ...
        $result = $this->Mdl_unmarried->deleteSource($id);

        if($result) {

            $this->session->set_flashdata('msg',"Success | ");

            redirect("unmarried-en/edit/".$main_id);
