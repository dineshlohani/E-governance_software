...
class Mdl_relationship extends CI_Model
{
    /*------------------------------------------------------------------------------------------------------------------*/
...
//        $this->db->where('type="fox" OR type="dog"');
    ...
    public function getAll()
    {
        if($this->session->userdata('is_muncipality') != 1) {
            $this->db->where('added_ward', $this->session->userdata('ward_no'));
        }
...
//        $this->db->where('type="fox" OR type="dog"');
    ...
    public function saverels($post_data) {
        ...
        $this->db->insert_batch('relation_members',$post_data);
    ...
    public function getRelationShipDetails($id){
        $query = $this->db->get_where('relation_members', array('main_id' => $id));
      ...
      public function updateMembers($id, $post_array) {
        ...
        $this->db->update_batch('relation_members',$post_array,'id');
    ...
    public function removeMember($id) {
        $this->db->where('id', $id);
        $del=$this->db->delete('relation_members');
        if($del) {
            return TRUE;
        } else {
            return FALSE;
