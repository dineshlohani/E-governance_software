function getCalendarDivString(a) {
  var b =
    '<div id="ndp-nepali-box" class="ndp-corner-all" style="display:none">';
  return (
    (b += '<span id="ndp-target-id" style="display:none"></span>'),
    (b += '<div class="ndp-corner-all ndp-header">'),
    (b +=
      '<a href="javascript:void(0)" id="prev" title="Previous Month" class="ndp-prev"></a>'),
    (b +=
      '<a href="javascript:void(0)" id="next" title="Next Month" class="ndp-next"></a>'),
...
function showNdpCalendarBox(a) {
  if (npdCalendarVisible) hideCalendarBox(!1);
    ...
    var b = jQuery("#" + a).val();
    jQuery("#ndp-target-id").html(a);
    var c = jQuery("#" + a).offset();
    jQuery("#ndp-nepali-box").css("top", c.top + jQuery("#" + a).outerHeight()),
      jQuery("#ndp-nepali-box").css("left", c.left),
      showCalendar(b, a),
      (npdCalendarVisible = !0);
  }
}
function setSelectedDay(a, my_sel_id) {
  var newvar = a.split(",");
  var a = newvar[0];
  var selected_id = newvar[1];
  checkMyDate(a, selected_id);
  var b = jQuery("#ndp-target-id").html();
  jQuery("#" + b).val(a),
    ndpAttr.ndpEnglishInput &&
      jQuery("#" + ndpAttr.ndpEnglishInput).val(BS2AD(a)),
    hideCalendarBox();
}
function showCalendar(a, my_sel_id) {
  window.my_sel_id = my_sel_id;
  jQuery("#ndp-nepali-box table").find("tr:gt(0)").remove(),
    "" === a
      ? jQuery("#ndp-nepali-box table").append(getDateTable(""))
      : jQuery("#ndp-nepali-box table").append(getDateTable(a)),
    "block" == jQuery("#ndp-nepali-box").css("display") &&
      jQuery("#ndp-nepali-box").hide(),
    jQuery("#ndp-nepali-box").fadeIn(100);
}
function getDateTable(a) {
  ...
  if ("" === a) {
    var d = getNepaliDate();
    return (
      (b = getMonthParameters(d)),
      (c = getDateRows(b[0], b[1], b[2], b[3], b[4]))
    );
  }
  return (
    (b = getMonthParameters(a)), (c = getDateRows(b[0], b[1], b[2], b[3], b[4]))
  );
}
function performSelectMonth(a) {
  showCalendar(
    (ndpAttr.npdYear ? jQuery("#npd-year-select").val() : a) +
      "-" +
      jQuery("#npd-month-select").val() +
      "-01"
  );
}
function performSelectYear(a) {
  showCalendar(
    jQuery("#npd-year-select").val() +
      "-" +
      (ndpAttr.npdMonth ? jQuery("#npd-month-select").val() : a) +
      "-01"
  );
}
function getMonthSelect(a, b) {
  var c = getNepaliMonthsInNepali(),
    d =
      '<select id="npd-month-select" onchange="performSelectMonth(' + b + ')">';
  return (
...
function getYearSelect(a, b, c) {
  console.log(c);
  var d =
    '<select id="npd-year-select" onchange="performSelectYear(' + a + ')">';
  for (
    i =
      Math.round(c / 2) > 0 && parseInt(b) - Math.round(c / 2) >= 2e3
        ? parseInt(b) - Math.round(c / 2)
        : 2e3;
    i <=
    (Math.round(c / 2) > 0 && parseInt(b) + Math.round(c / 2) <= 2090
      ? parseInt(b) + Math.round(c / 2)
      : 2090);
    i++
  )
...
function getMonthParameters(a) {
  ...
  ndpAttr.npdMonth && ndpAttr.npdYear
    ? jQuery("#currentMonth").html(
        getMonthSelect(d, c) + getYearSelect(d, c, e)
      )
    : ndpAttr.npdMonth
    ? jQuery("#currentMonth").html(
        getMonthSelect(d, c) + " " + englishNo2Nep(c)
      )
    : ndpAttr.npdYear
    ? jQuery("#currentMonth").html(
        getNepaliMonth(d - 1) + " " + getYearSelect(d, c, e)
      )
    : jQuery("#currentMonth").html(
        getNepaliMonth(d - 1) + "&nbsp;" + englishNo2Nep(c)
      ),
    (nYear = pYear = c),
    (nMonth = parseInt(d, 10) + 1),
    nMonth > 12 && (nYear++, (nMonth = "01")),
    (pMonth = parseInt(d, 10) - 1),
    pMonth < 1 && (pYear--, (pMonth = "12")),
    jQuery("#prev").attr(
      "onclick",
      "showCalendar('" + pYear + "-" + pMonth + "-01')"
    ),
    jQuery("#next").attr(
      "onclick",
      "showCalendar('" + nYear + "-" + nMonth + "-01')"
    );
  var f = b[2],
...
function getDateRows(a, b, c, d, e) {
  for (
    ...
    var m = l - a + 1,
      ...
      o = "";
    (o =
      c == h && get2DigitNo(d) == i && j == get2DigitNo(m)
        ? "ndp-selected"
        : m == e
        ? "ndp-current"
        : "ndp-date"),
      a > l
        ...
        : ((k += "<td class='" + o + "'>"),
          (k +=
            "<a href='javascript:void(0)' onclick=\"setSelectedDay('" +
            n +
            "," +
            my_sel_id +
            "')\">" +
...
function hideCalendarBox(a) {
  (a = "undefined" != typeof a ? a : !0),
    jQuery("#ndp-nepali-box").fadeOut(100),
...
function add_ad_days(a, b) {
  ...
  return (
    ...
    c.getFullYear() +
      "-" +
      (ad_month < 10 ? "0" + ad_month.toString() : ad_month) +
      "-" +
      (ad_day < 10 ? "0" + ad_day.toString() : ad_day)
  );
}
...
function ad2bs(a) {
  return (
    (days_count = this.count_ad_days(this.ad_date_eq, a)),
      ...
      (b = "undefined" != typeof b ? b : {}),
        ...
        this.each(function () {
          ...
          a(this).addClass("ndp-nepali-calendar"),
            (ndpData[c] = b),
            ndpAttr.onFocus !== !1 &&
              a(this).attr("onfocus", "showNdpCalendarBox('" + c + "')"),
            ndpAttr.ndpTriggerButton &&
              a(this).after(
                '<button type="button" class="ndp-click-trigger ' +
                  (ndpAttr.ndpTriggerButtonClass
                    ? ndpAttr.ndpTriggerButtonClass
                    : "") +
                  '" onclick="showNdpCalendarBox(&quot;' +
                  c +
                  '&quot;)">' +
                  (ndpAttr.ndpTriggerButtonText
                    ? ndpAttr.ndpTriggerButtonText
                    : "Pick Date") +
                  "</button>"
              );
        ...
        a(".ndp-nepali-calendar, #ndp-nepali-box").hover(
          function () {
            mouse_is_inside = !0;
          },
          function () {
        ...
        a("html").mouseup(function (b) {
          a(b.target).is(".ndp-click-trigger") ||
            (npdCalendarVisible && !mouse_is_inside && hideCalendarBox(!1));
        });
    };
  })(jQuery);
